...
data class StrategyRules(
    ...
    companion object {
        val DEFAULT = StrategyRules(
            version = "v7-profit-first-regime-risk-control",
            minimumScore = 80.0,
            maxResults = 2,
            validForMinutes = 180,
            entryBandPct = 0.08,
            candidateSelection = CandidateSelectionRules(
                maxCandleTargets = 70,
                topTradeValueCount = 80,
                topChangeRateCount = 55,
                volumeBuildupCount = 45,
                quietAccumulationCount = 12,
                medianTradeValueMultiplier = 1.0,
                minBuildupChangeRatePct = -2.0,
                maxBuildupChangeRatePct = 10.0,
                maxQuietAbsChangeRatePct = 1.6,
            ),
            compressionBreakout = CompressionBreakoutRules(
                enabled = true,
                rangeCompressionRatio = 0.65,
                maxDistanceTo15mHighPct = 1.4,
            ...
            trendPullback = TrendPullbackRules(
                ...
                min15mMaMultiplier = 1.001,
                minPriorLowMultiplier = 0.982,
                pullbackLookback = 10,
                reclaimLookback = 5,
            ),
            bearDecouplingBounce = BearDecouplingBounceRules(
                enabled = false,
                btcWeakBelowPct = -2.0,
                altStrongAbovePct = 2.5,
                maxTradeValueRank = 45,
                minFourHourVolumeMultiple = 1.8,
                maxPreviousFourHourVolumeMultiple = 3.2,
                maxPriceOver240mMa20Pct = 7.0,
                maxBearishUpperWickPct = 56.0,
            ...
            prePumpRotation = PrePumpRotationRules(
                enabled = true,
                minChange24hPct = -1.5,
                maxChange24hPct = 11.0,
                maxChange30mPct = 6.0,
                maxChange5mPct = 3.2,
                maxTradeValueRank = 55,
                maxChangeRank = 55,
                minRotation30mPct = 0.45,
                minVolumeAcceleration = 1.30,
                minFiveMinuteVolumeRatio = 1.28,
                minFifteenMinuteVolumeRatio = 1.18,
                maxRangePct = 6.8,
                minRangePosition = 0.55,
                minHighProximityMultiplier = 0.986,
                minCloseStairCount = 2,
            ),
            scoring = ScoringRules(
                overheat24hBasePct = 12.0,
                overheat24hWeight = 1.5,
                overheat30mBasePct = 4.0,
                overheat30mWeight = 2.4,
                overheat5mBasePct = 2.2,
                overheat5mWeight = 5.5,
                overheatMax = 38.0,
                hardBlockBtc24hBelowPct = -4.5,
                hardBlock30mPumpPct = 6.0,
                hardBlock5mPumpPct = 3.5,
                hardBlockRedUpperWickPct = 58.0,
            ),
            risk = RiskRules(
                defaultStopMultiplier = 0.989,
                minimumRiskPct = 0.38,
                minimumExpectedReturnPct = 0.85,
            ),
            customFlags = mapOf(
                "mode" to "profit-first-regime-risk-control",
                "reason" to "Tighten PRE_PUMP overheat gates, BTC hard block, compression quality, and keep bear decoupling disabled until positive expectancy is proven.",
            ),
        )
    ...
    companion object {
        fun fromJson(json: JSONObject?): CandidateSelectionRules {
            ...
            return CandidateSelectionRules(
                maxCandleTargets = json?.optInt("maxCandleTargets", d.maxCandleTargets)?.coerceIn(10, 100) ?: d.maxCandleTargets,
                topTradeValueCount = json?.optInt("topTradeValueCount", d.topTradeValueCount)?.coerceIn(10, 120) ?: d.topTradeValueCount,
                topChangeRateCount = json?.optInt("topChangeRateCount", d.topChangeRateCount)?.coerceIn(5, 100) ?: d.topChangeRateCount,
                volumeBuildupCount = json?.optInt("volumeBuildupCount", d.volumeBuildupCount)?.coerceIn(0, 100) ?: d.volumeBuildupCount,
                quietAccumulationCount = json?.optInt("quietAccumulationCount", d.quietAccumulationCount)?.coerceIn(0, 100) ?: d.quietAccumulationCount,
                medianTradeValueMultiplier = json?.optDouble("medianTradeValueMultiplier", d.medianTradeValueMultiplier)?.coerceIn(0.3, 3.0) ?: d.medianTradeValueMultiplier,
                ...
                maxBuildupChangeRatePct = json?.optDouble("maxBuildupChangeRatePct", d.maxBuildupChangeRatePct)?.coerceIn(0.0, 25.0) ?: d.maxBuildupChangeRatePct,
