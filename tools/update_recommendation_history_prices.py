...
def latest_session(symbol: str) -> tuple[float, str, float | None, float | None, list[float]]:
    ...
    recent_closes = [round(value, 6) for _, value in valid[-30:]]
    return current, current_date, previous, change_pct, recent_closes
...
def main() -> None:
    ...
    for row in payload.get("recommendations", []):
        ...
        try:
            ...
            current, current_date, previous, change_pct, recent_closes = latest_session(symbol)
            ...
            row["recentCloses"] = recent_closes
            updated += 1
